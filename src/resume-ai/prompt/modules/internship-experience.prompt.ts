...
任务目标
根据提供的职位描述（JD）、候选人原始经历以及当前日期，生成简历的internshipExperience模块JSON数据。输出必须严格遵循JSON格式，字段结构必须与下方给定的示例完全一致。
...
输出格式要求
输出必须是单一且有效的 JSON 对象，仅包含 internshipExperience 字段，不得包含任何额外的解释、注释、Markdown 代码块或其他文本。
...
1. 确认 internshipExperience 为数组类型
2. 确认每个经历对象包含所有必需字段
3. 确认 description 字段使用正确的 HTML 结构
